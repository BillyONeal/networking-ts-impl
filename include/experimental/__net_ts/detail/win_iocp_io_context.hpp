...
#ifndef NET_TS_DETAIL_WIN_IOCP_IO_CONTEXT_HPP
#define NET_TS_DETAIL_WIN_IOCP_IO_CONTEXT_HPP
...
public:
  ...
  NET_TS_DECL std::error_code register_handle(
      HANDLE handle, std::error_code& ec);

  // Run the event loop until stopped or no more work.
  NET_TS_DECL size_t run(std::error_code& ec);

  // Run until stopped or one operation is performed.
  NET_TS_DECL size_t run_one(std::error_code& ec);

  // Run until timeout, interrupted, or one operation is performed.
  NET_TS_DECL size_t wait_one(long usec, std::error_code& ec);

  // Poll for operations without blocking.
  NET_TS_DECL size_t poll(std::error_code& ec);
  ...
  NET_TS_DECL void post_deferred_completion(win_iocp_operation* op);
  ...
  NET_TS_DECL void on_pending(win_iocp_operation* op);

  // Called after starting an overlapped I/O operation that completed
  // immediately. The caller must have already called work_started() prior to
  // starting the operation.
  NET_TS_DECL void on_completion(win_iocp_operation* op,
  ...
  // starting the operation.
  NET_TS_DECL void on_completion(win_iocp_operation* op,
      const std::error_code& ec, DWORD bytes_transferred = 0);
...
private:
  ...
  // either 0 or 1).
  NET_TS_DECL size_t do_one(DWORD msec, std::error_code& ec);

  // Helper to calculate the GetQueuedCompletionStatus timeout.
  NET_TS_DECL static DWORD get_gqcs_timeout();
...
#endif // NET_TS_DETAIL_WIN_IOCP_IO_CONTEXT_HPP
