...
#ifndef NET_TS_DETAIL_IMPL_WIN_IOCP_IO_CONTEXT_IPP
#define NET_TS_DETAIL_IMPL_WIN_IOCP_IO_CONTEXT_IPP
...
std::error_code win_iocp_io_context::register_handle(
...
void win_iocp_io_context::on_completion(win_iocp_operation* op,
    const std::error_code& ec, DWORD bytes_transferred)
{
  // Flag that the operation is ready for invocation.
  op->ready_ = 1;
...
size_t win_iocp_io_context::do_one(DWORD msec, std::error_code& ec)
{
  for (;;)
  {
    ...
    if (overlapped)
    {
      ...
      {
        result_ec = std::error_code(static_cast<int>(op->Offset),
            *reinterpret_cast<std::error_category*>(op->Internal));
...
#endif // NET_TS_DETAIL_IMPL_WIN_IOCP_IO_CONTEXT_IPP
